...
## 기술 스택 (Tech Stack)
- [x] **Python**  
  - 프로젝트의 **기본 언어**로, 데이터 처리와 백엔드 로직 구현에 사용

- [x] **Streamlit**  
  - **웹 애플리케이션 프레임워크**로, LLM 기능을 시각적으로 제공하고 사용자 입력을 처리
...
- [x] **Ollama**  
  - 로컬 환경에서 LLM을 실행하고 관리하는 **AI 모델 서버**로, LangChain과 연동되어 모델 추론을 수행
        ┌───────────────┐
        │    Streamlit  │◀──── 사용자 입력/출력
        └───────┬───────┘
                │
        ┌───────▼───────┐
        │   LangChain   │─── 데이터 흐름 제어 / 체인 구성
        └───┬───────────┘
            │
 ┌──────────▼───────────┐
 │      Ollama (LLM)    │─── 모델 추론 수행
